...
import {
  Clock, CalendarDays, TrendingUp, Target, AlertTriangle, Building2,
} from "lucide-react"
        ...
        <DialogHeader className="border-b border-slate-100 px-6 pt-6 pb-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge variant="outline" className="bg-white gap-1">
              <Building2 className="size-3" /> {estagiario.empresa}
            </Badge>
            {estagiario.faltas > 0 && (
              ...
              <Table>
                <TableHeader className="sticky top-0 z-10 bg-white">
                  <TableRow className="hover:bg-transparent">
                    ...
                    <TableHead className="text-xs font-semibold text-slate-600">Observação</TableHead>
                ...
                <TableBody>
                  ...
                  {frequencias.map((f) => (
                    <TableRow key={f.id} className="hover:bg-slate-50/70">
                      <TableCell className="text-sm text-slate-700">{f.data}</TableCell>
                      ...
                      <TableCell className="text-xs text-slate-500">{f.observacao || "—"}</TableCell>
                      <TableCell className="text-right text-sm font-medium text-slate-700">{f.horas}h</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
